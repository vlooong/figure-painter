...
- **全流程覆盖** —— 从图表数据提取、到期刊级绘图、再到 Benchmark 查询，一站式完成论文图表与结果整理。
- **完全本地化** —— 纯前端运行，数据只存本机 IndexedDB；首次加载后可离线使用。
- **科研导向** —— 内置期刊模板、色盲友好配色、高 DPI 导出，面向投稿场景设计。
- **高效可复用** —— 支持数据集复用、对比表格一键复制到论文，提高整理效率。
...
### 1. 数据提取
1. 上传图表图片，放大或平移至可标定区域
2. 进行 4 点坐标轴标定，选择线性或对数坐标
3. 采样曲线颜色并调节容差，自动提取数据
4. 使用手动点选补齐数据，并通过叠图验证精度
5. 导出 CSV/Excel 或直接发送到绘图模块

### 2. 科研绘图
1. 选择期刊模板或配色方案
2. 添加或导入多个数据集，必要时启用双 Y 轴
3. 微调线型、符号、坐标轴、网格等样式
4. 以 PNG/SVG 导出，并设置 DPI 以满足投稿要求

### 3. Benchmark 查询
1. 根据任务类别筛选数据集与算法
2. 查看多预测长度下的 MSE/MAE 对比结果
3. 一键复制 LaTeX/TSV/Markdown 表格到论文中
